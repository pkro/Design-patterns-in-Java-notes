...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(results.first); // Complex(-2, 0)
    System.out.println(results.second); // Complex(-8, 0)
  }
}
...
class QuadraticEquationSolver {
  ...
  public Pair<Complex, Complex> solve(double a, double b, double c) {
    ...
    // well my sucky math only brought me this far, next 4 lines from solution
    Complex rootDisc = Complex.sqrt(discriminant);
    return new Pair<>(
        new Complex(-b, 0).plus(rootDisc).divides(new Complex(2, 0)).divides(new Complex(a, 0)),
        new Complex(-b, 0).minus(rootDisc).divides(new Complex(2, 0)).divides(new Complex(a, 0)));
  }
}
...
class Complex {
  ...
  public static Complex sqrt(double value) {
    if (value < 0) return new Complex(0, Math.sqrt(-value));
    else return new Complex(Math.sqrt(value), 0);
  }
